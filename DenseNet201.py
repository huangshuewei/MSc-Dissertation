...
# ResNet50
DenseNet201_base_model = DenseNet201(weights = "imagenet", include_top=False, input_shape=(512, 128,3))

DenseNet201_model = models.Sequential()

DenseNet201_model.add(DenseNet201_base_model)
DenseNet201_model.add(Flatten())
DenseNet201_model.add(Dense(512,
...
history_DenseNet201 = DenseNet201_model.fit(x_train_image, 
                          y_train_label_cat,
                          verbose=1, 
                          epochs=10,
...
loss_DenseNet201, acc_DenseNet201 = DenseNet201_model.evaluate(x_test_image, 
                           y_test_label_cat, 
                           batch_size=batch_size)


print('\n\nDenseNet201\n\nLoss     : {} \nAccuracy : {}'.format(history_DenseNet201.history['loss'][-1],history_DenseNet201.history['accuracy'][-1]))
print('\n\nDenseNet201\n\nLoss     : {} \nAccuracy : {}'.format(loss_DenseNet201,acc_DenseNet201))

DenseNet201_model.save("CNN_DenseNet201.h5")


# Test data DenseNet201
Y_pred = DenseNet201_model.predict(x_test_image)
...
plt.title('Receiver operating characteristic to multi-class (DenseNet201)')
...
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('Zoomed Receiver operating characteristic to multi-class (DenseNet201)')
plt.legend(loc="lower right")
plt.show()
