...
# VGG19
vgg19_base_model = VGG19(weights = "imagenet", include_top=False, input_shape=(512, 128,3))

vgg19_model = models.Sequential()

vgg19_model.add(vgg19_base_model)
vgg19_model.add(Flatten())
vgg19_model.add(Dense(512,
...
history_vgg19 = vgg19_model.fit(x_train_image, 
                          y_train_label_cat,
                          verbose=1, 
                          epochs=10,
...
loss_vgg19, acc_vgg19 = vgg19_model.evaluate(x_test_image, 
                           y_test_label_cat, 
                           batch_size=batch_size)


print('\n\nVGG19\n\nLoss     : {} \nAccuracy : {}'.format(history_vgg19.history['loss'][-1],history_vgg19.history['accuracy'][-1]))
print('\n\nVGG19\n\nLoss     : {} \nAccuracy : {}'.format(loss_vgg19,acc_vgg19))

vgg19_model.save("CNN_vgg19.h5")


# Test data vgg19
Y_pred = vgg19_model.predict(x_test_image)
...
plt.title('Receiver operating characteristic to multi-class (VGG19)')
...
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('Zoomed Receiver operating characteristic to multi-class (VGG19)')
plt.legend(loc="lower right")
plt.show()
