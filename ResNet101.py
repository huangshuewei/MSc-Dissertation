...
# ResNet50
ResNet101_base_model = ResNet101(weights = "imagenet", include_top=False, input_shape=(512, 128,3))

ResNet101_model = models.Sequential()

ResNet101_model.add(ResNet101_base_model)
ResNet101_model.add(Flatten())
ResNet101_model.add(Dense(512,
...
history_ResNet101 = ResNet101_model.fit(x_train_image, 
                          y_train_label_cat,
                          verbose=1, 
                          epochs=10,
...
loss_ResNet101, acc_ResNet101 = ResNet101_model.evaluate(x_test_image, 
                           y_test_label_cat, 
                           batch_size=batch_size)


print('\n\nResNet101\n\nLoss     : {} \nAccuracy : {}'.format(history_ResNet101.history['loss'][-1],history_ResNet101.history['accuracy'][-1]))
print('\n\nResNet101\n\nLoss     : {} \nAccuracy : {}'.format(loss_ResNet101,acc_ResNet101))

ResNet101_model.save("CNN_ResNet101.h5")


# Test data ResNet50
Y_pred = ResNet101_model.predict(x_test_image)
...
plt.title('Receiver operating characteristic to multi-class (ResNet101)')
...
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('Zoomed Receiver operating characteristic to multi-class (ResNet101)')
plt.legend(loc="lower right")
plt.show()
