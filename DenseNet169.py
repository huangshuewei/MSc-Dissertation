...
# ResNet50
DenseNet169_base_model = DenseNet169(weights = "imagenet", include_top=False, input_shape=(512, 128,3))

DenseNet1691_model = models.Sequential()

DenseNet1691_model.add(DenseNet169_base_model)
DenseNet1691_model.add(Flatten())
DenseNet1691_model.add(Dense(512,
                      kernel_initializer=initializers.random_normal(stddev=0.01),
                      bias_initializer=initializers.Zeros(),
                      activation='relu'))
DenseNet1691_model.add(Dropout(0.5))
DenseNet1691_model.add(Dense(512,
                      kernel_initializer=initializers.random_normal(stddev=0.01),
                      bias_initializer=initializers.Zeros(),
                      activation='relu'))
DenseNet1691_model.add(Dropout(0.5))
DenseNet1691_model.add(Dense(6,
                      kernel_initializer=initializers.random_normal(stddev=0.01),
                      bias_initializer=initializers.Zeros(),
                      activation='softmax'))
DenseNet1691_model.summary()

# compile both models
optimizer = K.optimizers.Adam(learning_rate=0.00001)
DenseNet1691_model.compile(optimizer = optimizer, loss = 'categorical_crossentropy', metrics=['accuracy'])

# Training model
batch_size = 16

history_DenseNet169 = DenseNet1691_model.fit(x_train_image, 
                          y_train_label_cat,
                          verbose=1, 
                          epochs=10,
...
loss_DenseNet169, acc_DenseNet169 = DenseNet1691_model.evaluate(x_test_image, 
                           y_test_label_cat, 
                           batch_size=batch_size)


print('\n\nDenseNet169\n\nLoss     : {} \nAccuracy : {}'.format(history_DenseNet169.history['loss'][-1],history_DenseNet169.history['accuracy'][-1]))
print('\n\nDenseNet169\n\nLoss     : {} \nAccuracy : {}'.format(loss_DenseNet169,acc_DenseNet169))

DenseNet1691_model.save("CNN_DenseNet169.h5")


# Test data ResNet152
Y_pred = DenseNet1691_model.predict(x_test_image)
...
plt.title('Receiver operating characteristic to multi-class (DenseNet169)')
...
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('Zoomed Receiver operating characteristic to multi-class (DenseNet169)')
plt.legend(loc="lower right")
plt.show()
