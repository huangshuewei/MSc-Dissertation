...
# ResNet50
DenseNet121_base_model = DenseNet121(weights = "imagenet", include_top=False, input_shape=(512, 128,3))

DenseNet121_model = models.Sequential()

DenseNet121_model.add(DenseNet121_base_model)
DenseNet121_model.add(Flatten())
DenseNet121_model.add(Dense(512,
...
history_DenseNet121 = DenseNet121_model.fit(x_train_image, 
                          y_train_label_cat,
                          verbose=1, 
                          epochs=10,
...
loss_DenseNet121, acc_DenseNet121 = DenseNet121_model.evaluate(x_test_image, 
                           y_test_label_cat, 
                           batch_size=batch_size)


print('\n\nDenseNet121\n\nLoss     : {} \nAccuracy : {}'.format(history_DenseNet121.history['loss'][-1],history_DenseNet121.history['accuracy'][-1]))
print('\n\nDenseNet121\n\nLoss     : {} \nAccuracy : {}'.format(loss_DenseNet121,acc_DenseNet121))

DenseNet121_model.save("CNN_DenseNet121.h5")


# Test data ResNet152
Y_pred = DenseNet121_model.predict(x_test_image)
...
plt.title('Receiver operating characteristic to multi-class (DenseNet121)')
...
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('Zoomed Receiver operating characteristic to multi-class (DenseNet121)')
plt.legend(loc="lower right")
plt.show()
