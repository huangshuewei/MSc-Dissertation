...
# 152
ResNet152_base_model = ResNet152(weights = "imagenet", include_top=False, input_shape=(512, 128,3))

ResNet152_model = models.Sequential()

ResNet152_model.add(ResNet152_base_model)
ResNet152_model.add(Flatten())
ResNet152_model.add(Dense(512,
...
ResNet152_model.compile(optimizer = optimizer, loss = 'categorical_crossentropy', metrics=['accuracy'])

# Training model
batch_size = 16

history_ResNet152 = ResNet152_model.fit(x_train_image, 
                          y_train_label_cat,
                          verbose=1, 
                          epochs=10,
...
loss_ResNet152, acc_ResNet152 = ResNet152_model.evaluate(x_test_image, 
                           y_test_label_cat, 
                           batch_size=batch_size)


print('\n\nResNet152\n\nLoss     : {} \nAccuracy : {}'.format(history_ResNet152.history['loss'][-1],history_ResNet152.history['accuracy'][-1]))
print('\n\nResNet152\n\nLoss     : {} \nAccuracy : {}'.format(loss_ResNet152,acc_ResNet152))

ResNet152_model.save("CNN_ResNet152.h5")


# Test data ResNet152
Y_pred = ResNet152_model.predict(x_test_image)
...
plt.title('Receiver operating characteristic to multi-class (ResNet152)')
...
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('Zoomed Receiver operating characteristic to multi-class (ResNet152)')
plt.legend(loc="lower right")
plt.show()
