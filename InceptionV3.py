...
# InceptionV3
InceptionV3_base_model = InceptionV3(weights = "imagenet", include_top=False, input_shape=(512, 128,3))
...
history_InceptionV3 = InceptionV3_model.fit(x_train_image, 
                          y_train_label_cat,
                          verbose=1, 
                          epochs=10,
...
loss_InceptionV3, acc_InceptionV3 = InceptionV3_model.evaluate(x_test_image, 
                           y_test_label_cat, 
                           batch_size=batch_size)


print('\n\nInceptionV3\n\nLoss     : {} \nAccuracy : {}'.format(history_InceptionV3.history['loss'][-1],history_InceptionV3.history['accuracy'][-1]))
print('\n\nInceptionV3\n\nLoss     : {} \nAccuracy : {}'.format(loss_InceptionV3,acc_InceptionV3))

InceptionV3_model.save("CNN_InceptionV3.h5")


# Test data DenseNet201
Y_pred = InceptionV3_model.predict(x_test_image)
...
plt.title('Receiver operating characteristic to multi-class (InceptionV3)')
...
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('Zoomed Receiver operating characteristic to multi-class (InceptionV3)')
plt.legend(loc="lower right")
plt.show()
