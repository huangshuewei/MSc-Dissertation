...
# ResNet50
ResNet50_base_model = ResNet50(weights = "imagenet", include_top=False, input_shape=(512, 128,3))

ResNet50_model = models.Sequential()

ResNet50_model.add(ResNet50_base_model)
ResNet50_model.add(Flatten())
ResNet50_model.add(Dense(512,
...
history_ResNet50 = ResNet50_model.fit(x_train_image, 
                          y_train_label_cat,
                          verbose=1, 
                          epochs=10,
...
loss_ResNet50, acc_ResNet50 = ResNet50_model.evaluate(x_test_image, 
                           y_test_label_cat, 
                           batch_size=batch_size)


print('\n\nResNet50\n\nLoss     : {} \nAccuracy : {}'.format(history_ResNet50.history['loss'][-1],history_ResNet50.history['accuracy'][-1]))
print('\n\nResNet50\n\nLoss     : {} \nAccuracy : {}'.format(loss_ResNet50,acc_ResNet50))

ResNet50_model.save("CNN_ResNet50.h5")


# Test data ResNet50
Y_pred = ResNet50_model.predict(x_test_image)
...
plt.title('Receiver operating characteristic to multi-class (ResNet50)')
...
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('Zoomed Receiver operating characteristic to multi-class (ResNet50)')
plt.legend(loc="lower right")
plt.show()
