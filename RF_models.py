...
acc_tr = []
i = 0
df = pd.DataFrame(columns = ['n_estimators','max_depth','min_samples_leaf','train_acc','test_acc'])

for c in [15]:
    for g in [14]:
        for h in [2]:
            clf = RandomForestClassifier(n_estimators=c, max_depth=g,
                                     min_samples_leaf=h, criterion="entropy",
...
plt.title("Confusion Matrix (Random Forest)", size = 30)
plt.show()
...
plt.title('Receiver operating characteristic to multi-class (Random Forest)')
